...
void BugContourDefiner::VectorCW(Point& point) // сдвиг вектора ПО часовой стрелке, каждая координата может иметь значения ТОЛЬКО -1, 0 и 1
{
  int& X = point.x;
  int& Y = point.y;
...
void BugContourDefiner::VectorCCW(Point& point) // сдвиг вектора ПРОТИВ часовой стрелке, каждая координата может иметь значения ТОЛЬКО -1, 0 и 1
{
  int& X = point.x;
  int& Y = point.y;
